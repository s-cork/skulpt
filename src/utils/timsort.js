 ...
 * @extends pyObject
 */
export function timSort(list, length) {
    this.list = new pyList(list.v);
...
timSort.prototype.lt = function (a, b) {
    return objectRichCompare(a, b, "Lt");
};

timSort.prototype.le = function (a, b) {
    return !this.lt(b, a);
};

timSort.prototype.setitem = function (item, value) {
 ...
 */
timSort.prototype.binary_sort = function (a, sorted) {
    var pivot;
    var p;
    var r;
    var l;
...
timSort.prototype.count_run = function (a) {
    /*
    ...
    return {"run": new pyListSlice(a.list, a.base, n), "descending": descending};
};
...
timSort.prototype.sort = function () {
    ...
    var cr;
    var sorted;
    var remaining = new pyListSlice(this.list, 0, this.listlength);
    if (remaining.len < 2) {
        return;
    }
    ...
    asserts.assert(remaining.base == this.listlength);

    this.merge_force_collapse();
    asserts.assert(this.pending.length == 1);
    asserts.assert(this.pending[0].base === 0);
    asserts.assert(this.pending[0].len == this.listlength);
};
 ...
 */
timSort.prototype.gallop = function (key, a, hint, rightmost) {
    var lower;
    var self;
    var p;
    var lastofs;
    ...
    var hintminlastofs;
    var m;
    asserts.assert(0 <= hint && hint < a.len);
    self = this;
    if (rightmost) {
        lower = function (a, b) {
            return self.le(a, b);
    ...
    asserts.assert(-1 <= lastofs < ofs <= a.len);
    ...
    asserts.assert(lastofs == ofs);         // so a[ofs-1] < key <= a[ofs]
    return ofs;
};

// ____________________________________________________________

timSort.prototype.merge_init = function () {
...
timSort.prototype.merge_lo = function (a, b) {
    var min_gallop;
    var dest;
    var acount, bcount;
    var p;
    asserts.assert(a.len > 0 && b.len > 0 && a.base + a.len == b.base);
    ...
    try {
        ...
        asserts.assert(a.len >= 0 && b.len >= 0);
...
timSort.prototype.merge_hi = function (a, b) {
    var min_gallop;
    var dest;
...
timSort.prototype.merge_at = function (i) {
    var a;
    var b;
    var k;
    if (i < 0) {
    ...
    b = this.pending[i + 1];
    asserts.assert(a.len > 0 && b.len > 0);
    asserts.assert(a.base + a.len == b.base);

    // Record the length of the combined runs and remove the run b
    this.pending[i] = new pyListSlice(this.list, a.base, a.len + b.len);
...
timSort.prototype.merge_collapse = function () {
    var p = this.pending;
    while (p.length > 1) {
...
timSort.prototype.merge_force_collapse = function () {
    var p = this.pending;
    while (p.length > 1) {
...
timSort.prototype.merge_compute_minrun = function (n) {
 ...
 * @extends pyObject
 */
function pyListSlice(list, base, len) {
    this.list = list;
    this.base = base;
    this.len = len;
};

pyListSlice.prototype.copyitems = function () {
    //Make a copy of the slice of the original list
    var start = this.base;
    var stop = this.base + this.len;
    asserts.assert(0 <= start <= stop);
    return new pyListSlice(new pyList(this.list.v.slice(start, stop)), 0, this.len);
};

pyListSlice.prototype.advance = function (n) {
    this.base += n;
    this.len -= n;
    asserts.assert(this.base <= this.list.sq$length());
};

pyListSlice.prototype.getitem = function (item) {
    return this.list.v[item];
};

pyListSlice.prototype.setitem = function (item, value) {
    this.list.v[item] = value;
};

pyListSlice.prototype.popleft = function () {
...
pyListSlice.prototype.popright = function () {
    this.len--;
    return this.list.v[this.base + this.len];
};

pyListSlice.prototype.reverse = function () {
