    ...
    eval_$rw$: {
        ...
        $meth: function (source, globals, locals) {
            ...
            return Sk.misceval.chain(Sk.builtin.eval(source, tmp_globals, tmp_locals), (res) => {
                if (locals !== undefined) {
                    Object.entries(tmp_locals).forEach(([key, val]) => {
                        locals.mp$ass_subscript(new Sk.builtin.str(Sk.unfixReserved(key)), val);
                    });
                }
                if (globals !== undefined) {
                    Object.entries(tmp_globals).forEach(([key, val]) => {
                        globals.mp$ass_subscript(new Sk.builtin.str(Sk.unfixReserved(key)), val);
                    });
                }
                return res;
            });
    ...
    exec: {
        $meth: function (source, globals, locals) {
            ...
            return Sk.misceval.chain(Sk.builtin.exec(source, tmp_globals, tmp_locals), (new_locals) => {
                if (locals !== undefined) {
                    Object.entries(new_locals).forEach(([key, val]) => {
                        locals.mp$ass_subscript(new Sk.builtin.str(Sk.unfixReserved(key)), val);
                    });
                }
                if (globals !== undefined) {
