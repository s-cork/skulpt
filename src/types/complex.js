 ...
 *
 */
export var pyComplex = buildNativeClass("complex", {
    constructor: function complex(real, imag) {
        if (!(this instanceof pyComplex)) {
            throw new TypeError("bad call to complex constructor, use 'new'");
        }
        asserts.assert(typeof real === "number" && typeof imag === "number", "complex arguments must be Js Numbers");
        this.real = real;
        this.imag = imag;
    },
    slots: /**@lends {pyComplex.prototype}*/ {
        tp$as_number: true,
        tp$doc:
        ...
        tp$hash: function () {
            ...
            if (pyInt.withinThreshold(v)) {
                return new pyInt(parseInt(v, 10));
            }
            return new pyInt(JSBI.BigInt(v));
        },
        tp$getattr: genericGetAttr,
        tp$new: function (args, kwargs) {
            args = keywordArrayToNamedArgs("complex", ["real", "imag"], args, kwargs, [null, null]);
            return complex_from_py.call(this, args[0], args[1]);
        },
        tp$richcompare: function (w, op) {
            if (op !== "Eq" && op !== "NotEq") {
                if (checkNumber(w) || checkComplex(w)) {
                    throw new pyExc.TypeError("no ordering relation is defined for complex numbers");
                }
        ...
        // number slots
        nb$int_: function () {
            throw new pyExc.TypeError("can't convert complex to int");
        },
        nb$lng: function () {
            throw new pyExc.TypeError("can't convert complex to long");
        },
        nb$float_: function () {
            throw new pyExc.TypeError("can't convert complex to float");
        },
        ...
        nb$add: complexNumberSlot((a_real, a_imag, b_real, b_imag) => {
            return new pyComplex(a_real + b_real, a_imag + b_imag);
        }),
        ...
        nb$reflected_subtract: complexNumberSlot((a_real, a_imag, b_real, b_imag) => {
            return new pyComplex(b_real - a_real, b_imag - a_imag);
        }),
        nb$multiply: complexNumberSlot((a_real, a_imag, b_real, b_imag) => {
            return new pyComplex(b_real * a_real - b_imag * a_imag, a_real * b_imag + a_imag * b_real);
        }),
        ...
        nb$floor_divide: function (other) {
            throw new pyExc.TypeError("can't take floor of complex number.");
        },
        nb$reflected_floor_divide: function (other) {
            throw new pyExc.TypeError("can't take floor of complex number.");
        },
        nb$remainder: function (other) {
            throw new pyExc.TypeError("can't mod complex numbers.");
        },
        nb$reflected_remainder: function (other) {
            throw new pyExc.TypeError("can't mod complex numbers.");
        },
        nb$divmod: function (other) {
            throw new pyExc.TypeError("can't take floor or mod of complex number.");
        },
        nb$power: function (other, z) {
            if (z != null && !checkNone(z)) {
                throw new pyExc.ValueError("complex modulo");
            }
        ...
        nb$abs: function () {
            ...
            if (!_is_finite(_real) || !_is_finite(_imag)) {
                ...
                */
                if (_is_infinity(_real)) {
                    return new pyFloat(Math.abs(_real));
                } else if (_is_infinity(_imag)) {
                    return new pyFloat(Math.abs(_imag));
                }
            ...
            if (!_is_finite(result)) {
                throw new pyExc.OverflowError("absolute value too large");
            }
    ...
    getsets: {
        real: {
            $get: function () {
                return new pyFloat(this.real);
            },
            $doc: "the real part of a complex number",
        },
        imag: {
            $get: function () {
                return new pyFloat(this.imag);
            },
            $doc: "the imaginary part of a complex number",
        },
    },
    methods: /**@lends {pyComplex.prototype}*/ {
        conjugate: {
            $meth: function () {
                return new pyComplex(this.real, -this.imag);
            },
        ...
        __format__: {
            $meth: function (format_spec) {
                ...
                throw new pyExc.TypeError("__format__ requires str");
            },
            $flags: { OneArg: true },
            $textsig: null,
...
function try_complex_special_method(op) {
    ...
    const f = objectLookupSpecial(op, pyStr.$complex);
    if (f !== undefined) {
...
function complex_from_py(real, imag) {
    ...
    if (r != null && r.constructor === pyComplex && i == null) {
    ...
    if (checkString(r)) {
        if (i != null) {
            throw new pyExc.TypeError("complex() can't take second arg if first is a string");
        }
        return pyComplex.complex_subtype_from_string(r, this);
    }

    if (i != null && checkString(i)) {
        throw new pyExc.TypeError("complex() second arg can't be a string");
    }

    // try_complex_special_method
    tmp = try_complex_special_method(r);
    if (tmp != null && tmp !== pyNotImplemented) {
        if (!checkComplex(tmp)) {
            throw new pyExc.TypeError("__complex__ should return a complex object");
        }
        r = tmp;
    }
    ...
    if (r != null) {
        if (!check_number(r)) {
            throw new pyExc.TypeError("complex() first argument must be a string or a number, not '" + typeName(r) + "'");
        }
    }

    if (i != null) {
        if (!check_number(i)) {
            throw new pyExc.TypeError("complex() second argument must be a number, not '" + typeName(r) + "'");
        }
    }
 ...
 * @function
 *
 * @return {pyComplex} an instance of complex - could be a subtype's instance
 *
 * @param {number} real
 * @param {number} imag
 * @param {Object} type_prototype pyComplex.prototype
 * @ignore
 */
function complex_subtype_from_doubles(real, imag, type_prototype) {
    if (type_prototype === pyComplex.prototype) {
        return new pyComplex(real, imag);
    } else {
        const instance = new type_prototype.constructor();
        pyComplex.call(instance, real, imag);
        return instance;
    }
}
 ...
 * We leave this as pyComplex since it is called by the compiler
 * @ignore
 */
pyComplex.complex_subtype_from_string = function (val, type_prototype) {
    type_prototype = type_prototype || pyComplex.prototype;
    var index;
    var start;
    ...
    if (val.indexOf("\0") !== -1 || val.length === 0 || val === "") {
        throw new pyExc.ValueError("complex() arg is a malformed string");
    }
    ...
    if (match !== null) {
        ...
        if (val[index] === "j" || val[index] === "J") {
            ...
            if (val[index] !== "j" && val[index] !== "J") {
                throw new pyExc.ValueError("complex() arg is malformed string");
            }
...
function divide(a_real, a_imag, b_real, b_imag) {
    ...
    if (abs_b_real >= abs_b_imag) {
        ...
        if (abs_b_real === 0.0) {
            throw new pyExc.ZeroDivisionError("complex division by zero");
        } else {
            ratio = b_imag / b_real;
...
function c_powu(a_real, a_imag, n) {
    var r, p; // Py_complex
    let mask = 1;
    r = new pyComplex(1.0, 0.0);
    p = new pyComplex(a_real, a_imag);

    while (mask > 0 && n >= mask) {
        if (n & mask) {
            r = new pyComplex(r.real * p.real - r.imag * p.imag, r.real * p.imag + p.real * r.imag);
        }

        mask <<= 1;
        p = new pyComplex(p.real * p.real - p.imag * p.imag, 2 * p.real * p.imag);
    }
 ...
 *
 * This functions assumes, that v is always instance of pyComplex
 * @ignore
 */
...
function _PyComplex_FormatAdvanced(self, format_spec) {
    throw new pyExc.NotImplementedError("__format__ is not implemented for complex type.");
}
...
function PyOS_double_to_string(val, format_code, precision, flags, type) {
    ...
    if (isNaN(val)) {
        ...
        format_str = new pyStr(format_str);
        /**
         ...
         */
        buf = format_str.nb$remainder(new pyFloat(val));
