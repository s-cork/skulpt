 ...
 * You should use pyCall to call the Python function.
 *
 ...
 *
 */
export var pyFunc = buildNativeClass("function", {
    constructor: function pyFunc(code, globals, closure, closure2) {
        if (!this instanceof pyFunc) {
            throw new TypeError("builtin func should be called as a class with `new`");
        }
        this.func_code = code;
        this.func_globals = globals || null;

        this.$name = (code.co_name && code.co_name.v) || code.name || "<native JS>";
        this.$d = pyDict ? new pyDict() : undefined;
        this.$doc = code.$doc;
        this.$module = (sysGlobals && sysGlobals["__name__"]) || pyNone;
        ...
        if (code.co_fastcall) {
            ...
            this.tp$call = pyFunc.prototype.tp$call; // keep func the same shape
        }
    },
    slots: {
        ...
        $r() {
            return new pyStr("<function " + this.$qualname + ">");
        },
        tp$call(posargs, kw) {
            ...
            if (this.co_argcount === undefined && this.co_varnames === undefined && !this.co_kwargs && !this.func_closure) {
                ...
                // resolving anything.
                if (kw && kw.length !== 0) {
                    throw new pyExc.TypeError(this.$name + "() takes no keyword arguments");
                }
    ...
    getsets: {
        __name__: {
            ...
            $set(value) {
                if (!checkString(value)) {
                    throw new pyExc.TypeError("__name__ must be set to a string object");
                }
        ...
        __qualname__: {
            $get() {
                return new pyStr(this.$qualname);
            },
            $set(value) {
                if (!checkString(value)) {
                    throw new pyExc.TypeError("__qualname__ must be set to a string object");
                }
        ...
        __defaults__: {
            $get() {
                return new pyTuple(this.$defaults);
...
pyFunc.prototype.$resolveArgs = function (posargs, kw) {
    ...
    if (this.co_varargs) {
        ...
        args[totalArgs] = new pyTuple(vararg);
        ...
        for (let i = 0; i < kw.length; i += 2) {
            ...
            if (idx >= 0) {
                if (args[idx] !== undefined) {
                    throw new pyExc.TypeError(this.$name + "() got multiple values for argument '" + name + "'");
                }
                args[idx] = value;
            } else if (kwargs) {
                kwargs.push(new pyStr(name), value);
            } else {
                throw new pyExc.TypeError(this.$name + "() got an unexpected keyword argument '" + name + "'");
            }
        }
    }
